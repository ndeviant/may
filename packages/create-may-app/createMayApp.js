...
const program = new commander.Command(packageJson.name)
	...
	.option(
		...
		"use a non-standard version of may-tasks",
	)
	.option("--use-npm")
	.option("--use-pnp")
	.allowUnknownOption()
	.on("--help", () => {
		...
		console.log(
			`      - a custom fork published on npm: ${chalk.green("my-may-tasks")}`,
		);
		console.log(
		...
		console.log(
			`      - a .tgz archive: ${chalk.green(
				"https://mysite.com/my-may-tasks-0.8.2.tgz",
			)}`,
		);
		console.log(
			`      - a .tar.gz archive: ${chalk.green(
				"https://mysite.com/my-may-tasks-0.8.2.tar.gz",
			)}`,
		);
		console.log(
		...
		console.log(
			`      ${chalk.cyan("https://github.com/ndeviant/may/issues/new")}`,
		);
		console.log();
	})
	.parse(process.argv);
		...
		.run(
			{
				...
				npmPackages: ["may-tasks", "bootstrap", "svgxuse"],
				npmGlobalPackages: ["create-may-app"],
			},
			{
				duplicates: true,
				showNotFound: true,
...
if (typeof projectName === "undefined") {
	...
	console.log(`  ${chalk.cyan(program.name())} ${chalk.green("my-may-app")}`);
	console.log();
	console.log(
...
function createApp(name, verbose, version, useNpm, usePnp, template) {
	...
	console.log(`Creating a new May app in ${chalk.green(root)}.`);
	console.log();
...
function getInstallPackage(version, originalDirectory) {
	let packageToInstall = "may-tasks";
...
function getPackageName(installPackage) {
	if (installPackage.match(/^.+\.(tgz|tar\.gz)$/)) {
		return getTemporaryDirectory()
			...
			.catch(err => {
				// The package name could be with or without semver version, e.g. may-tasks-0.2.0-alpha.1.tgz
	...
	if (installPackage.indexOf("git+") === 0) {
		...
		// git+https://github.com/mycompany/may-tasks.git
		// git+ssh://github.com/mycompany/may-tasks.git#v1.2.3
...
function checkAppName(appName) {
	...
	const dependencies = ["may-tasks", "bootstrap", "svgxuse"].sort();
